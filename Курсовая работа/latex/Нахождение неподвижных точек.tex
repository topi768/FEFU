...
\title{ВЛИЯНИЕ РАВНОВЕСНОГО ПРОМЫСЛА НА СЦЕНАРИИ РАЗВИТИЯ ДВУХВОЗРАСТНОЙ ПОПУЛЯЦИИ ПРИ ПЛОТНОСТНОЙ РЕГУЛЯЦИИ РОЖДАЕМОСТИ}
\author{Я}
\maketitle
...
$ 1 = s \alpha - s \alpha\beta Y -s \alpha u_1 + s \alpha \beta u_1Y + v
 - s \alpha u_2 + s \alpha \beta u_2 Y + s \alpha \beta u_1 u_2- s \alpha u_1 u_2 Y - v u_2 \\$
...
Пусть числитель Y будет равен N, а знаменатель D. Тогда $ Y = \dfrac{N}{D} \\$
$ N = s \alpha (1-u_1)(1-u_2) + v(1-u_2)-1 \\$
$ D = s \alpha \beta (1-u_1)(1-u_2) \\$
После подстановки и сокращения получаю
$  X = \dfrac{N(1 - v(1 - u_2))}{s^2 \alpha \beta (1 - u_1)(1 - u_2)^2} \\$
% $$ J = \begin{pmatrix} 
%     X'_x & X'_y \\ 
%     Y'_x & Y'_y
%  \end{pmatrix} \\$$
...
Построю матрицу Якоби или же мтрицу возмущений:
 $$ J = \begin{pmatrix} 
    F'_{1X} & F'_{1Y} \\ 
    F'_{2X} & F'_{2Y}
 \end{pmatrix}
  = \begin{pmatrix} 
    0 & \alpha(1-u_1)(1-2\beta Y) \\ 
    s(1-u_1) & v(1-u_2)
 ...
 Теперь построю треугольник устойчивости, краями которого есть 3 прямые: \\
   1) $ q = 1 - p (\lambda = 1)$ - \textbf{касательная бифуркация (fold)}\\
   2) $ q = 1 - p (\lambda = -1)$ - \textbf{бифуркация удвоения периода (flip)}\\
   3) $ q = 1 $ ($\lambda$ - комплексное(?)) - \textbf{бифуркация Неймарка–Сакера} \\
